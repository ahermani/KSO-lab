...
PRIVATE int chosenAlgorithm = 0; /* 0 - First Fit, 1 - Worst Fit */
 ...
 */
 ...
 /* FIRST FIT */
  ...
  do
    {
      hp = max = hole_head;	
      prev_max = prev_ptr = NIL_HOLE; /* dziura poprzednia do maksymalnej potrzebna przy usuwaniu */
      while(hp != NIL_HOLE && hp->h_base < swap_base)
      {
        if(hp->h_len > max->h_len)	/* jesli rozmiar obecnej dziury jest wiekszy od maksa */
        {
          max = hp; /* zapamietaj maksa */
          prev_max = prev_ptr; /* zapamietaj dziure przed maksem */
      ...
      /* znaleziono maksa i jest wystarczajaco duzy */
      if(max != NIL_HOLE && max->h_len >= clicks)
      {
        old_base = max->h_base;
...
{
  struct hole *hp;
  phys_clicks buffer[NR_HOLES * 2 + 1]; /* zaalokuj maksymalna mozliwa ilosc miejsca - wszystkie pary + zero */
  unsigned int i = 0;
  unsigned int nbytes = mm_in.m1_i1;
  unsigned int pairs = (nbytes/sizeof(phys_clicks))/2;		/* ile par zmiesci sie w buforze */
  
  /* jesli nie zmieszcza sie pary, wstaw zero na koniec bufora */
  if(pairs == 0)
	goto end;

  /* jesli nie zmiesci sie nawet zero, zwroc -1 */
  if(pairs < 0)
	return -1;

  for(hp = hole_head; i < pairs && hp != NIL_HOLE && hp->h_base < swap_base; hp = hp->h_next)
  {
    buffer[i] = hp->h_len;			/* zapisz rozmiar dziury */
    buffer[i+1] = hp->h_base;			/* zapisz adres dziury */
    i+=2;
  }

  end:
  buffer[i] = 0;						/* na koniec wstaw 0 */

  sys_copy(MM_PROC_NR, D,(phys_clicks) buffer, who, D, (phys_clicks) mm_in.m1_p1, (phys_clicks) nbytes);
  return i/2;						/* zwroc liczbe par */
}
...
int do_worst_fit()
{
  int choice = mm_in.m1_i1;
  
  switch(choice)
  {
    case 0:
    case 1:
	chosenAlgorithm = choice;
	break;
    default:
