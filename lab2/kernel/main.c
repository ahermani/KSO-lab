...
/* Start the ball rolling. */
  ...
  for (rp = BEG_PROC_ADDR, t = -NR_TASKS; rp < END_PROC_ADDR; ++rp, ++t) {
	rp->p_nr = t;		/* proc number from ptr */
        ...
        rp->BASE_PRI = MIN_PRI; /* inicjalizacja priorytetow dla nowych procesow */
	rp->ACT_PRI = MIN_PRI;  /* inicjalizacja priorytetow dla nowych procesow */
  }
  ...
  proc[NR_TASKS+INIT_PROC_NR].BASE_PRI = MIN_PRI; /* inicjalizacja priorytetow dla init */
  proc[NR_TASKS+INIT_PROC_NR].ACT_PRI = MIN_PRI; /* inicjalizacja priorytetow dla init */
