 ...
 */
  register struct proc* current = rdy_head[USER_Q];
  ...
  /*jesli kolejka pusta, wstaw na poczatek i koniec jednoczesnie*/
  ...
  /* szukamy procesu o priorytecie nizszym niz nowy proces */
  while(current->p_nextready != NIL_PROC && rp->ACT_PRI <= current->p_nextready->ACT_PRI){
	current = current->p_nextready;
  }

  /* jesli proces ma wyzszy pri od glowy, wstaw przed glowe */
  if (rp->ACT_PRI > rdy_head[USER_Q]->ACT_PRI){
	rp->p_nextready = rdy_head[USER_Q];
	rdy_head[USER_Q] = rp;
	pick_proc();
  }

  /* jesli proces ma nizszy pri od ogona, wstaw za ogon */
  else if (current->p_nextready == NIL_PROC){
      rdy_tail[USER_Q]->p_nextready = rp;
      rdy_tail[USER_Q] = rp;
      rdy_tail[USER_Q]->p_nextready = NIL_PROC;	
  }
  
  /* jesli ma pri pomiedzy glowa a ogonem, wstaw w odpowiednie miejsce */
  else {
  	rp->p_nextready = current->p_nextready;
 ...
 */
  register struct proc* current = rdy_head[USER_Q];
  register struct proc* temp_head;

  if (rdy_head[USER_Q] == NIL_PROC) return;

  printf("PID: %d PRI: %d\n", rdy_head[USER_Q]->p_pid, rdy_head[USER_Q]->BASE_PRI);

  rdy_head[USER_Q]->ACT_PRI = rdy_head[USER_Q]->BASE_PRI;

  /*proces starzenia*/
  if (rdy_head[USER_Q]->BASE_PRI >= MIN_PRI && rdy_head[USER_Q]->BASE_PRI <= MAX_AGE){
	  while (current->p_nextready != NIL_PROC) {
		current = current->p_nextready;
		if (current->ACT_PRI >= MIN_PRI && current->ACT_PRI < MAX_AGE)
			current->ACT_PRI++;
	  }
  }

  current = rdy_head[USER_Q];
  /* szukamy procesu o pri wyzszym niz glowa */
  while(current->p_nextready != NIL_PROC && rdy_head[USER_Q]->ACT_PRI <= current->p_nextready->ACT_PRI){
	current = current->p_nextready;
  }
  /* jesli nie ma takiego, wstaw glowe na koniec, kolejny jest glowa */
  if (current->p_nextready == NIL_PROC){
	rdy_tail[USER_Q]->p_nextready = rdy_head[USER_Q];
	rdy_tail[USER_Q] = rdy_head[USER_Q];
	rdy_head[USER_Q] = rdy_head[USER_Q]->p_nextready;
	rdy_tail[USER_Q]->p_nextready = NIL_PROC;	
  }
  /* jesli jest, ustaw go jako glowe*/
  else if(rdy_head[USER_Q]->ACT_PRI <= rdy_head[USER_Q]->p_nextready->ACT_PRI){
  	temp_head = rdy_head[USER_Q]->p_nextready;
	rdy_head[USER_Q]->p_nextready = current->p_nextready;
	current->p_nextready = rdy_head[USER_Q];
	rdy_head[USER_Q] = temp_head;
  }
