...
#define NLEN (sizeof(rp->p_name)-1)
  ...
  rp->p_reg.pc = (reg_t) m_ptr->IP_PTR;	/* set pc */
	rp->p_sel_alarm = 0;		/* reset select alarm timer */
...
/* Handle sys_xit().  A process has exited. */
  ...
  rc->p_alarm = 0;		/* turn off alarm timer */
	rc->p_sel_alarm = 0;		/* turn off select alarm timer */
...
PRIVATE int do_setpri(message *m_ptr){
  ...
  int option = m_ptr->m1_i2;
  int value = (int) m_ptr->m1_i3;
  switch(option) {
  /* ustaw priorytet biezacego procesu */
  case 1 :
	for(pp = BEG_USER_ADDR; pp < END_PROC_ADDR; ++pp){
		if(pp->p_pid == m_ptr->m1_i1) break;
	}
	pp->BASE_PRI = value;
  break;
  /* ustaw zmienna MAX_AGE */
  case 2 :
	MAX_AGE = value;
  break;
  /* ustaw zmienna MIN_PRI */
  case 3 :
	MIN_PRI = value;
  break;
  }
